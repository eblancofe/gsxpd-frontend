/**
 * ---------------------------------------------------------------
 *  Archivo:        expediente-informe-anual.component.ts
 *  Descripción:    COMPONENTE->Realiza, genera informes filtrados por Año de cierre del Expediente. 
 *                  Carga expedientes desde el backend, obtiene los expedientes disponibles por años 
 *                  y permite filtrar, visualizar e imprimir/exportar los resultados en PDF.
...
import html2pdf from 'html2pdf.js'; //Guardar salida en archivo en PDF
...
@Component({
  selector: 'app-expediente-informe-anual',
  standalone: true,
  templateUrl: './expediente-informe-anual.component.html',
  styleUrl: './expediente-informe-anual.component.scss',
  imports: [
    CommonModule, 
    FormsModule,
    ReactiveFormsModule,
...
export class ExpedienteInformeAnualComponent implements OnInit {
  ...
  filtrarPorAnio() { //Filtra expedientes por año seleccionado
    ...
    /*
    console.log("FILTRADOS:", this.expedientes);
    console.log("EXPEDIENTES ORIGINALES:", this.expedientesOriginales);
  ...
  imprimir() { //Imprime la tabla de informe/s en una nueva ventana
    ...
    <html>
      <head>
        <title>Informes de Expedientes por Año</title>
        <style>
          @page {
            size: A4 landscape;
            margin: 10mm;
  ...
  descargarPDF() { //Descarga el informe como PDF
    ...
    const options = {
      margin: 10,
      filename: `informe_anual_${this.anioSeleccionado}.pdf`,
      image: { type: 'jpeg' as const, quality: 0.98 },
      html2canvas: { scale: 2 },
      jsPDF: { unit: 'mm' as const, format: 'a4' as const, orientation: 'portrait' as const }
    };

    html2pdf() //Genera y posiblita la descarga el PDF
      .from(element)
      .set(options)
      .save()
      .then(() => {
